...
void			DisposeWiredPtr(Ptr inPtr);
...
void			DisposeHandle(Handle inHandle);
...
*/
NewtonErr	CheckHeap(
					Heap		inHeap,				// heap to check (nil for current heap)
					void **	outWhereSmashed);	// if non-nil and heap is smashed, address of accident
...
int		NextHeapBlock(
				...
				void *		inFromBlock,			// from block, nil for first block
...
void		CountHeapBlocks(
				...
				Heap		inHeap,				// heap to visit (nil for current heap)
...
void	SetRandomNilReturns(
			Heap	inHeap,			// heap to return nil on (nil for current heap)
