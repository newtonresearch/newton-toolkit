...
- (void)viewWillAppear
{
	...
	NSMutableDictionary * projectItems = doc.projectItems;
	if (self.representedObject == nil) {
		...
		sidebarItems = projectNode.mutableChildNodes;
		NSArray * items = [projectItems objectForKey:@"items"];
		...
		NSInteger selItem = [(NSNumber *)[projectItems objectForKey:@"selectedItem"] integerValue] + 1;
...
- (int)filetype:(NSURL *)inURL
{
	...
	if (filetype == gTypeNames.count)
		filetype = -1;	// we didn’t recognise the type of file!
	return filetype;
}
...
- (void) outlineView: (NSOutlineView *) inView willDisplayCell: (NSCell *) inCell forTableColumn: (NSTableColumn *) inColumn item: (id) inItem
{
	...
	imageAndTextCell.image = item.image;
}
...
- (void) outlineViewSelectionDidChange: (NSNotification *) inNotification
{
	...
	[wc editDocument:((NTXProjectItem *)theNode.representedObject).document];	// accessing document lazily instantiates viewController if necessary
}
...
- (void) addNewDataToSelection: (NTXProjectItem *) newChildData
{
	...
	if (parentNode == sourceListRoot) parentNode = nil;
